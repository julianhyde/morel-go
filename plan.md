...
### F. Standard-library breadth (51-74)
...
matches morel-java's — save for three cross-cutting features that
are deferred and then added once, across every file, at the end
of the phase (tasks 72-74): `Sys.plan` output (stubbed), postfix
method calls (`x.f a`, morel#346), and `op` sections (an operator
used as a first-class value). A structure's commit is titled
"`X` structure", and its message states the truth plainly — e.g.
"String structure is complete, and matches the Standard ML Basis
except for postfix method calls and op sections." Complete means
the *only* remaining divergence from java is those three kinds;
anything else is an unfinished member or feature, not done.
...
`StringCvt`'s radix, so a minimal `StringCvt` precedes them.
...
- [ ] 53. `String` structure — complete the built-in file: the
      last members and printed forms that java's tests exercise.
- [ ] 54. `List` structure — complete.
- [ ] 55. `Bag` structure — implement the `'a bag` value type and
...
- [ ] 56. `StringCvt` structure — enough to unblock `fmt`: the
...
- [ ] 57. `Int` structure — complete; `Int.fmt` uses task 56's
      radix.
- [ ] 58. `Real` structure — complete; `Real.fmt` likewise.
- [ ] 59. `Math` structure — complete.
- [ ] 60. `Option` structure — complete.
- [ ] 61. `Bool` structure — complete.
- [ ] 62. `Char` structure — complete.
- [ ] 63. `General` structure — complete.
- [ ] 64. `Vector` structure — complete (finish task 52: the
      `Vector;` dump now prints, given task 51's label quoting and
      contiguous type-variable numbering).
- [ ] 65. `ListPair` structure — `zip`, `unzip`, `map`, `app`,
      `all`, `exists`, `foldl`/`foldr`, and the `*Eq` variants
      that raise `UnequalLengths`.
- [ ] 66. `Either` structure — the `('a, 'b) either` datatype
      (`INL`/`INR`); `isLeft`/`isRight`, `asLeft`/`asRight`,
      `map`/`mapLeft`/`mapRight`, `app`/`appLeft`/`appRight`,
      `fold`, `proj`, `partition`.
- [ ] 67. `Fn` structure — combinators `id`, `const`, `apply`,
      `o`, `curry`, `uncurry`, `flip`, `repeat`, `equal`,
      `notEqual`.
- [ ] 68. `Range` structure — range values (morel#338);
      `contains`, `toList`/`toBag`, `discreteSetOf`/
      `continuousSetOf`, `flatten`, `ranges`, `complement`.
- [ ] 69. `Variant` structure — variant values (morel#324);
      `parse`, `print`.
- [ ] 70. `Date` structure — the `date` type over Go's `time`
      ...
      are deterministic.
- [ ] 71. `Time` structure — the `time` type (morel#351, #352);
      `fromReal`/`toReal`, `to`/`fromSeconds`…`Nanoseconds`,
      `compare`, `now`, `fmt`, `toString`/`fromString`,
      `zeroTime`.

Three cross-cutting features were deferred by every structure
above; each closes the last of the per-structure divergence in
one pass over every file, so it is done once, here, at the end:

- [ ] 72. Postfix method calls (morel#346): `x.f a` desugars to
...
- [ ] 73. `op` sections: an operator used as a first-class value
      (`(op @)`, `op +` passed to a fold). A `pull-passing.py`
      run adds the op hunks everywhere.
- [ ] 74. `Sys.plan`: emit java's compiled-plan string. A
