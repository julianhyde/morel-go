...
// Outer joins (morel#75). A "left join" makes the newly scanned
// variable option-typed, a "right join" the variables already in
// scope, and a "full join" both; the "on" condition sees the
// unwrapped types. A right or full join's source must be
// independent of the query's input, because it generates rows
// when no input row matches.
...
// toSafeNav lowers "arg?.field" to functor maps, as java's
// Resolver does: one map per layer (Option.map, List.map,
// Bag.map, Vector.map), the innermost function being the field
// selector, so NONE stays NONE and SOME r becomes SOME (#field r).
