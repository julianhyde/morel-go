...
func constructors(sys *types.System, name string) []types.Constructor {
	cons := sys.Constructors(name)
...
func domainSize(sys *types.System, t types.Type) *big.Int {
	...
	case *types.Named:
		total := big.NewInt(0)
		for _, con := range constructors(sys, t.Name) {
			if con.Arg == nil {
...
func ordinalOf(sys *types.System, t types.Type, v Val) *big.Int {
	...
	case *types.Named:
		...
		total := big.NewInt(0)
		for _, c := range constructors(sys, t.Name) {
