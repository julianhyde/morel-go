...
// Args is the parsed command line, mirroring the subset of java's
// Shell options that morel-go supports.
type Args struct {
...
// ParseArgs parses a command line into Args. Following java's
// Shell, an unrecognized flag is ignored rather than rejected, so
// that options morel-go does not implement (--foreign,
// --color-scheme, --system) are tolerated. A bare
...
func ParseArgs(argv []string) *Args {
	...
	for i := 0; i < len(argv); i++ {
		...
		case strings.HasPrefix(arg, "-"):
			// Unknown flag; ignored, as java's Shell does.
		default:
